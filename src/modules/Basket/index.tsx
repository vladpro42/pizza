...
import { selectItems } from "./api/basket.slice"
import OrderItem from "./components/OrderItem"
import { removeAllItemsFromCart } from "./api/basket.slice"
import { getTotalPrice } from "./utils/utils"
import { Link } from "react-router-dom"
import img from "../../assets/img/basket_img.svg"
...
const Basket = () => {
    ...
    if (pizzas.items.length === 0) {
        return <div className="basket__wrapper_clear">
            ...
            <img className="basket_img" src={img} alt="" />
            <Link to={"../"} className="nav__back_clear">Вернуться назад</Link>
        </div>
    }
    return <div className="basket__wrapper">
        <div className="top">
            ...
            <button onClick={() => dispatch(removeAllItemsFromCart())} className="basket__clear-btn">
        ...
        <div className="basket__nav">
            <Link to={"../"} className="nav__back">&lsaquo;<span>Вернуться назад</span></Link>
            <button className="nav__pay">Оплатить сейчас</button>
        </div>
    </div>
}
